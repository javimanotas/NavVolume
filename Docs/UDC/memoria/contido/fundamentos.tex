...
En consecuencia, el presente capítulo se estructura en tres bloques.
...
En segundo lugar, se describirán las herramientas y el entorno de desarrollo empleados a lo largo del proyecto, desde el control de versiones hasta la integración continua.
Por último, se profundizará en los fundamentos teóricos, los cuales establecerán las bases de las estructuras de datos y las técnicas algorítmicas empleadas en el proyecto.
...
% Colores de las líneas (se reutilizan ficblue y udcpink del estilo del documento)
\definecolor{lineBlue}{RGB}{0,114,189}
\definecolor{lineOrange}{RGB}{217,83,25}
\definecolor{lineGreen}{RGB}{34,139,34}
\definecolor{linePurple}{RGB}{126,47,142}
...
\begin{figure}[htp]
  ...
  \begin{tikzpicture}
    \begin{axis}[
      ...
      % DOD: línea continua y marcador relleno // POO (OOP): línea discontinua y marcador hueco
      \addplot[lineGreen,   solid,  mark=*]                              coordinates {(1000,6.50) (10000,3.00) (100000,3.00) (1000000,3.20) (10000000,3.00)};
      \addlegendentry{Intel Core i5-6267U DOD}
      \addplot[lineGreen, dashed, mark=o,        mark options={solid}] coordinates {(1000,3.50) (10000,5.00) (100000,16.50) (1000000,16.00) (10000000,13.00)};
      \addlegendentry{Intel Core i5-6267U OOP}

      \addplot[udcpink,  solid,  mark=square*]                        coordinates {(1000,4.50) (10000,4.00) (100000,2.50) (1000000,2.50) (10000000,2.50)};
      \addlegendentry{Intel Core i7 7700HQ DOD}
      \addplot[udcpink,    dashed, mark=square,   mark options={solid}] coordinates {(1000,12.00) (10000,14.50) (100000,12.00) (1000000,5.00) (10000000,5.50)};
      \addlegendentry{Intel Core i7 7700HQ OOP}

      \addplot[lineBlue, solid,  mark=triangle*]                      coordinates {(1000,2.50) (10000,2.00) (100000,2.00) (1000000,2.00) (10000000,2.00)};
      \addlegendentry{Intel Core i7 8700K DOD}
      \addplot[lineBlue,    dashed, mark=triangle, mark options={solid}] coordinates {(1000,20.00) (10000,3.00) (100000,23.50) (1000000,15.50) (10000000,15.50)};
      \addlegendentry{Intel Core i7 8700K OOP}

    \end{axis}
  \end{tikzpicture}
  \caption[Rendimiento de \acrshort{poo} frente a \acrshort{dod}.]
  {Tiempo de ejecución de las implementaciones basadas en \acrshort{dod} y en \acrshort{poo} para distintos tamaños del conjunto de datos, medido en tres procesadores Intel~\cite{albano_dod}.
  Las líneas continuas corresponden al \acrshort{dod} y las discontinuas a la \acrshort{poo}.}
...
Estas técnicas de bajo nivel se aplican de forma recurrente a lo largo del Capítulo~\ref{chap:desarrollo}.
A modo de ejemplo, la manipulación de bits resulta esencial en la codificación compacta de las hojas y los punteros de la estructura de datos espacial (Sección~\ref{sec:iter-representacion}). El acceso por referencia de solo lectura (\texttt{ref readonly}) se emplea para optimizar las rutas más críticas del buscador (Sección~\ref{sec:iter-api-opt}).
Por último, la reserva de memoria en el \gls{stack} mediante \texttt{stackalloc}, junto con su manipulación a través de estructuras \texttt{Span\textless T\textgreater}, se aprovecha en el solucionador de evasión local, que se ejecuta en cada frame del sistema por lo que se debe evitar generar presión sobre el recolector de basura (Sección~\ref{sec:iter-evasion}).

\section{Herramientas y entorno de desarrollo}
\label{sec:herramientas}

Aparte de las tecnologías que se emplean en el sistema, su desarrollo requirió un conjunto de herramientas que conformasen un entorno de trabajo productivo.
A continuación se recogen todas aquellas que fueron empleadas a lo largo del proyecto:

\begin{itemize}
    \item \textbf{Entorno de programación:} la creación de escenas, la configuración de los componentes y la depuración visual se realizan sobre el editor del motor Unity~\cite{unity}, mientras que la escritura del código fuente en C\# se apoya en el entorno de desarrollo integrado Visual Studio~\cite{visualstudio}, que ofrece asistencia a la edición, navegación por el código y depuración.
    \item \textbf{Control de versiones:} la evolución del código se gestiona con el sistema de control de versiones distribuido Git~\cite{git}, empleando la plataforma GitHub~\cite{github} como servicio de alojamiento remoto del repositorio y soporte para la revisión e integración de los cambios.
    \item \textbf{Integración continua:} la verificación automática del proyecto se delega en un sistema de \acrfull{ci} implementado mediante GitHub Actions~\cite{githubactions}, que ejecuta de forma desatendida la batería de pruebas en una máquina remota ante cada propuesta o integración de cambios.
    \item \textbf{Calidad y estilo del código:} la consistencia del formato se garantiza con el formateador CSharpier~\cite{csharpier}, que aplica de manera automática una guía de estilo común a todo el código fuente.
    \item \textbf{Pruebas automatizadas:} la validación del comportamiento se construye sobre el Unity Test Framework~\cite{unity_test_framework}, que permite definir pruebas tanto en modo de edición (\textit{EditMode}) como en modo de ejecución (\textit{PlayMode}).
\end{itemize}
...
Al estudiar el ámbito de la navegación volumétrica, se puede observar que existen tres alternativas predominantes~\cite{Brewer2015NavMesh}.
Estas se clasifican en:

\begin{itemize}
    \item \textbf{Puntos de ruta (\textit{Waypoints}):} Esta solución requiere el posicionamiento manual de una serie de nodos que reciben dicho nombre.
    ...
    \item \textbf{Árboles octales (\textit{\Glspl{octree}}):} Constituyen la técnica más extendida en la industria.
...
Para evitar reconstruir la representación del espacio navegable cada vez que se carga el juego, los sistemas de navegación suelen proveer un proceso denominado como \gls{bake}~\cite{RecastNavigation, UnityNavMesh, UnrealNavSystem}.
