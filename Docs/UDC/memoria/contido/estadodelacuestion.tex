...
Entre estas soluciones, aquellas que gozan de mayor relevancia y adopción en la industria, se fundamentan en enfoques que combinan estructuras \acrshort{svo}~\cite{Brewer2015NavMesh} y el algoritmo de búsqueda heurística \gls{astar}~\cite{Millington}, conceptos ya explicados previamente en la Sección~\ref{sec:representacionespacionavegable}.
...
El principal atractivo de Mercuna radica en su capacidad para unificar, bajo un mismo ecosistema, casi todos los casos de uso que engloba la navegación.
...
Mercuna dispone de distintas licencias que dependerán del subconjunto de características requerido, con costes que parten de \pounds\,15,000 o de \pounds\,2,500 para estudios con un presupuesto de desarrollo inferior a \$\,500,000~\cite{MercunaLicensing}.
...
Esto se traduce en herramientas mucho más intuitivas para el desarrollador final.

En este ámbito predomina el ecosistema de Unity, respaldado por su tienda oficial de recursos: la Asset Store~\cite{assetstore}.
...
Como consecuencia de la vasta magnitud de este ecosistema, casi todas las herramientas de terceros se desarrollan de manera exclusiva para este motor.
...
Desarrollada por Softbrix Studio, Nav3D es la opción mejor valorada en la tienda de Unity (Asset Store)~\cite{Nav3D_SoftbrixAssetStore}.
...
Creada por el desarrollador independiente m6io, esta solución se comercializa con un enfoque estrictamente técnico.
...
En consecuencia, presenta una curva de aprendizaje mucho más pronunciada, exigiendo al usuario un nivel avanzado de programación en el lenguaje C\#~\cite{Nav3D_FlyingAIAssetStore}.
...
Esta tecnología permite compilar código C\# en código máquina muy optimizado mediante el uso de la infraestructura \gls{llvm}~\cite{llvm2004}.
...
Este estudio se estructura en un bloque de \textbf{análisis interno} (orientado a identificar las fortalezas y debilidades inherentes a la propuesta), un bloque de \textbf{análisis externo} (centrado en identificar las oportunidades y amenazas que presenta el mercado actual) y un apartado de conclusiones.
...
A pesar de que el proyecto deberá competir con soluciones comerciales maduras, su naturaleza de código abierto representa una gran ventaja competitiva.
En conclusión, el proyecto se posiciona como una alternativa democratizadora, accesible y con un claro potencial de adopción dentro del vasto ecosistema de desarrolladores de Unity.
