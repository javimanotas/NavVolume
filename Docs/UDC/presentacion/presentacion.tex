...
\begin{frame}[plain]
  ...
  \includegraphics[height=1.2cm]{udc.png}\\[0.7em]
  ...
  \begin{columns}[T]
    \begin{column}{0.58\textwidth}
      \begin{itemize}
        ...
        \item Dotar a los personajes no jugables de inteligencia.
        \item Soluciones basadas sobre la biblioteca Recast.
...
\begin{frame}{Estudio de mercado: Análisis DAFO}
  \centering
  \vspace{0.5em}
  ...
  \small
  \textbullet\ Código abierto.\newline
  \textbullet\ Transparencia total. & 
  \small
  \textbullet\ Alternativas consolidadas.\newline
  \textbullet\ Necesidad de adaptarse.\\ \hline
  \rowcolor{udcpink!25}
  \textbf{Oportunidades} & \textbf{Amenazas} \\ \hline
  \small
  \textbullet\ Carencia de herramientas integradas.\newline
  \textbullet\ Gran canal de difusión. & 
  \small
  \textbullet\ Sector en constante cambio.\newline
  \textbullet\ Motores emergentes.\newline
  \textbullet\ Riesgo de herramienta oficial. \\ \hline
  \end{tabular}}
\end{frame}
...
\begin{frame}{Fundamentos}
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      \begin{itemize}
        \setlength{\itemsep}{1.6em}
        \item \textbf{Representación del espacio con SVO}:
        \begin{itemize}
          \item Estructura jerárquica (octree disperso).
          \item Divide el espacio recursivamente solo donde hay geometría o información.
          \item Alta compresión de memoria y acceso eficiente.
        \end{itemize}
        \item \textbf{Búsqueda de caminos con A*}:
        \begin{itemize}
          \item Algoritmo heurístico para encontrar la ruta óptima.
          \item Se adapta a la resolución heterogénea del SVO.
          \item Permite avanzar grandes distancias en nodos vacíos con un solo salto.
        \end{itemize}
      \end{itemize}
    \end{column}
    \begin{column}{0.45\textwidth}
      ...
      \begin{tikzpicture}[font=\small]
        % Rellenar celdas ocupadas
        \fill[ocupado] (0,0) rectangle (2,2);
        \fill[ocupado] (2,2) rectangle (3,3);
        \fill[ocupado] (3.5,3.5) rectangle (4,4);
        
        % Cuadrado principal (Nivel 0)
        \draw[celda, thick] (0,0) rectangle (4,4);
        ...
        \draw[celda] (3.5,3) -- (3.5,4);
        \draw[celda] (3,3.5) -- (4,3.5);
...
\begin{frame}{Desarrollo: Iteración 0, arquitectura e integración continua}
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      \footnotesize
      \begin{itemize}
        \item \textbf{Arquitectura inicial sólida}: antes de implementar funcionalidades, se establece un esqueleto robusto.
        \item Tres \textit{assemblies} separados: \textbf{Runtime} (lógica central), \textbf{Editor} (herramientas) y \textbf{Tests}.
        \item \textbf{Regla de dependencias}: \textit{Editor} y \textit{Tests} dependen de \textit{Runtime}, pero nunca a la inversa.
        \item \textbf{Integración continua (CI)}: barrera de calidad desde el primer día. Validaciones automáticas en cada contribución:
        \begin{itemize}\footnotesize
          \item Verificación de formato (Csharpier).
          \item Batería de pruebas (\textit{EditMode} y \textit{PlayMode}).
        \end{itemize}
      \end{itemize}
    \end{column}
    \begin{column}{0.5\textwidth}
      ...
      \begin{tikzpicture}[
        ...
        \node[modulo, fill=ficblue!15, draw=ficblue,
          ...
          (editor) {\textbf{Editor}\\[2pt]\footnotesize Herramientas de\\configuración};
        \node[modulo, fill=udcgray!40, draw=udcgray!80!black,
              above=16mm of runtime.north east, anchor=south east]
          (tests) {\textbf{Tests}\\[2pt]\footnotesize Pruebas automatizadas};
