...
\begin{frame}[plain]
  ...
  \includegraphics[height=1.7cm]{udc.png}\\[0.7em]
...
\begin{frame}{El punto de partida: la navegación autónoma}
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
      \begin{itemize}
        \item La navegación autónoma es la base de la inteligencia de los personajes no jugables (NPC).
        \item El \textit{pathfinding} está prácticamente resuelto para agentes \alert{terrestres} en escenarios estáticos.
        \item Los motores integran soluciones nativas (Unity NavMesh, Unreal, Godot), casi todas sobre la biblioteca Recast.
  ...
  \begin{columns}[T]
    \begin{column}{0.58\textwidth}
      \begin{itemize}
        ...
        \item Añadir la altura como dimensión libre dispara el coste en tiempo y memoria, salvo con estructuras especializadas (octrees).
        \item Sin soporte oficial, los estudios recurren a soluciones \textit{ad-hoc}, caras y poco reutilizables.
      \end{itemize}
      \vspace{0.4em}
      \begin{block}{Necesidad}
        Un sistema \textbf{genérico, optimizado y reutilizable} que resuelva la navegación 3D de forma estructural.
      \end{block}
    \end{column}
...
\begin{frame}{Objetivos}
  \textbf{Objetivo general:} diseño, implementación y validación en Unity de un sistema de navegación volumétrica eficiente y optimizado, con \alert{enfoque de producto}: un paquete modular, de código abierto y listo para producción.
  \vspace{0.5em}
  \begin{columns}[T]
  ...
  \begin{columns}[T]
    \begin{column}{0.58\textwidth}
      \begin{itemize}
        ...
        \item \textbf{Físicas (PhysX)}: la geometría se consulta con \texttt{OverlapBox}, agrupadas en lotes y ejecutadas en paralelo.
        \item \textbf{DOTS / diseño orientado a datos}: Job System, \textbf{Burst} (LLVM + SIMD), \texttt{Unity.Mathematics} y colecciones nativas.
        \item \textbf{Bajo nivel en C\#}: \texttt{ref readonly}, \texttt{Span<T>}, \texttt{stackalloc} y manipulación de bits para minimizar el recolector de basura.
      \end{itemize}
    \end{column}
    \begin{column}{0.42\textwidth}
      \centering
      \includegraphics[width=\textwidth]{dodvsoop.png}\\[0.3em]
...
\begin{frame}{Fundamentos teóricos}
  \begin{columns}[T]
    ...
    \begin{column}{0.4\textwidth}
      \centering
      \includegraphics[width=0.92\textwidth]{zfold.png}\\[0.2em]
      {\scriptsize\color{udcgray} Curva de Morton (versión 2D).}\\[0.6em]
      \includegraphics[width=0.92\textwidth]{catmullrom.png}\\[0.2em]
...
\begin{frame}{Planificación y seguimiento}
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      \footnotesize
      \textbf{Planificación inicial}
      \begin{itemize}\footnotesize
        \item 360 h (créditos ECTS), 14 semanas (marzo--junio de 2026).
        \item Fases T1--T7 con cinco hitos (H1--H5).
      \end{itemize}
      \vspace{0.2em}
      \textbf{Seguimiento}
      \begin{itemize}\footnotesize
        \item La robustez geométrica (\textit{clipping}) costó más de lo previsto.
        \item La holgura recuperada permitió añadir la \alert{evasión local} (T8), no prevista.
        \item Proyecto final: 15 semanas y \textbf{400 h} reales.
      \end{itemize}
    \end{column}
    \begin{column}{0.5\textwidth}
      \centering
      {\footnotesize\textbf{Coste estimado del proyecto}}\\[0.4em]
      ...
      \end{tabular}}\\[0.5em]
      {\scriptsize\color{udcgray} Frente a los 4.984,52\,\euro\ estimados al inicio.}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Iteraciones 5 y 6: optimización y evasión local}
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      \footnotesize
      \textbf{Iteración 5: optimización}
      \begin{itemize}\footnotesize
    ...
    \begin{column}{0.5\textwidth}
      \footnotesize
      \textbf{Iteración 6: evasión local (ORCA)}
      \begin{itemize}\footnotesize
...
\begin{frame}{Rendimiento: construcción y memoria del volumen}
  ...
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      ...
      \begin{tikzpicture}
        \begin{axis}[
            ybar stacked, width=\textwidth, height=4.6cm, bar width=18pt,
          ...
          \addplot[fill=udcgray!50, draw=udcgray!65!black] coordinates {(8,97) (9,534) (10,1855)};
        \end{axis}
    ...
    \begin{column}{0.5\textwidth}
      ...
      \begin{tikzpicture}
        \begin{axis}[
            ybar, width=\textwidth, height=4.6cm, bar width=22pt,
...
\begin{frame}{Rendimiento: búsqueda de rutas y evasión}
  \begin{columns}[T]
    ...
    \begin{column}{0.5\textwidth}
      ...
      \begin{tikzpicture}
        \begin{axis}[
            ybar, width=\textwidth, height=4.6cm, bar width=8pt,
